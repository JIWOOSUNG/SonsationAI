...
criterion = nn.CrossEntropyLoss(
    weight=weights.to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
)
...
model = SignBERT(input_dim=225, num_classes=num_classes, max_seq_len=MAX_FRAMES).to(
    device
)
...
for epoch in range(EPOCHS):
    ...
    if acc > best_acc:
        best_acc = acc
        torch.save(
            {"model_state_dict": model.state_dict(), "label_map": dataset.label_map},
            MODEL_SAVE_PATH,
        )
